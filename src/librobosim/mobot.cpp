...
int CMobot::moveWait(void) {
	// wait for motion to complete
	MUTEX_LOCK(&_success_mutex);
	while ((_motor[JOINT1].success + _motor[JOINT2].success + _motor[JOINT3].success + _motor[JOINT4].success) != NUM_DOF) {
...
bool CMobot::isHome(void) {
    return ( fabs(_motor[JOINT1].theta) < EPSILON && fabs(_motor[JOINT2].theta) < EPSILON && fabs(_motor[JOINT3].theta) < EPSILON && fabs(_motor[JOINT4].theta) < EPSILON );
}
...
int CMobot::draw(osg::Group *root, int tracking) {
	...
	osg::ref_ptr<osg::Texture2D> tex = new osg::Texture2D(osgDB::readImageFile(g_sim->_tex_path + "mobot/body.png"));
...
void CMobot::draw_bigwheel(conn_t conn, osg::Group *robot) {
	...
	// apply texture
	osg::ref_ptr<osg::Texture2D> tex = new osg::Texture2D(osgDB::readImageFile(g_sim->_tex_path + "mobot/conn.png"));
	tex->setFilter(osg::Texture2D::MIN_FILTER,osg::Texture2D::LINEAR_MIPMAP_LINEAR);
	tex->setFilter(osg::Texture2D::MAG_FILTER,osg::Texture2D::LINEAR);
	tex->setWrap(osg::Texture::WRAP_S, osg::Texture::REPEAT);
	tex->setWrap(osg::Texture::WRAP_T, osg::Texture::REPEAT);
