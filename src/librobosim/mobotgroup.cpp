...
int CMobotGroup::moveJointTime(robotJointId_t id, double seconds) {
...
int CMobotGroup::moveJointTimeNB(robotJointId_t id, double seconds) {
	robots_t rtmp = _robots;
	while (rtmp) {
		rtmp->robot->moveJointTimeNB(id, seconds);
		rtmp = rtmp->next;
	}
