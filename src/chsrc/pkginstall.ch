...
/*******************************************************************
pkginstall.ch version 1.3.0, December 9, 2008
 *) added the following command for Linux
      chcon -t texrel_shlib_t $chpackage/$(pkgname)/dl/*.dl
pkginstall.ch version 1.2.0, May 17, 2006
 *) fixed installation in C:/Program Files/Ch
pkginstall.ch version 1.1.1, June 23, 2005
 *) fixed installation in HOME/pkgname in Unix
pkginstall.ch version 1.1.0, May 23, 2005
 *) If the user has no permission to install pkgname in 
    CHHOME/package/pkgname, install pkgname in HOME/pkgname. 
...
*******************************************************************/
/* 
     By default, pkginstall installs a package pkgname in the current 
     directory  into the <CHHOME>/package/pkgname and copies required header 
     files in pkgname/include into the CHHOME/toolkit/include  
     directory. If the user has no permission to install pkgname in
     CHHOME/package/pkgname, install pkgname in HOME/pkgname.
...
int main(int argc, char **argv) {
   ...
   if(u_option == false && d_option == false && access(chhome, W_OK)) {/* no write permision for CHHOME */
     fprintf(stderr, "You have no write permision in the Ch home directory '%s'.\n", chhome);
     fprintf(stderr, "Do you want to quit and login as root to retry (Y or N) [N]\n");
     scanf("%c", &check);
     if (check=='y' || check == 'Y') {
        exit(0);
     }
     scanf("%c", &check); /* get rid of '\n' */
     fprintf(stderr, "Install '%s' in '%s/%s' (Y or N)[N]? \n", pkgname, getenv("HOME"), pkgname);
     scanf("%c", &check);
     if (!(check=='y' || check == 'Y')) {
        fprintf(stderr, "Please follow the instruction below for installation.\n");
        fprintf(stderr, "  pkginstall.ch -d TARGET_DIRECTORY %s\n", pkgname);
        exit(0);
     }
     if(access(getenv("HOME"), W_OK)) {/* no write permision for HOME */
        fprintf(stderr, "Error: you do no have write permission to %s\n", getenv("HOME"));
     }
     else {
       ./pkginstall.ch -d $(getenv("HOME")) $pkgname 
     }
     exit(0);
   }
   ...
   if(u_option)  /* uninstall package */
   {
      ...
      else
      {
         ...
         foreach(token; installedfiles; NULL; "\n")
         {
            rm -rf "$token"
         }
