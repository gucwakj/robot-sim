...
objects.  Everything to be added is put between the
\verb%<ground>% tags.
\begin{verbatim}
<ground>
</ground>
...
of each other.  There are there sub-tags which specify the global attributes of
...
accessories available are shown in Table
\ref{tab:acc}.
...
\newline
Accessories with multiple sides are configured with that any number of
\verb%<side/>% tags as shown in the bridge example.
\begin{verbatim}
<bridge>
    <side id="1" robot="0" face="1"/>
...
connector with an attached smallwheel attached to the third face of the robot.
\begin{verbatim}
<simple>
